...
class GATKMuTect2(Module):
    ...
    def define_input(self):
        ...
        self.add_argument("cosmic",             is_required=True, is_resource=True)
    ...
    def define_command(self, platform):
        ...
        cosmic  = self.get_arguments("cosmic").get_value()
        ...
        opts = list()
        if is_tumor[0]:
            opts.append("-I:tumor %s" % bam[0])
            opts.append("-I:normal %s" % bam[1])
        else:
            opts.append("-I:tumor %s" % bam[1])
            opts.append("-I:normal %s" % bam[0])
        opts.append("-o %s" % vcf)
        opts.append("-R %s" % ref)
        opts.append("--dbsnp %s" % dbsnp)
        opts.append("--cosmic %s" % cosmic)
        ...
        cmd = "%s %s -jar %s -T MuTect2 %s !LOG3!" % (java, jvm_options, gatk, " ".join(opts))
        return cmd
