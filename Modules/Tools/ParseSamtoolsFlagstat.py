...
class ParseSamtoolsFlagstat(Module):

    def __init__(self, module_id):
        super(ParseSamtoolsFlagstat, self).__init__(module_id)
    ...
    def define_output(self, platform, split_name=None):
        ...
        summary_file = self.generate_unique_file_name(split_name=split_name, extension=".flagstat.qc_report.txt")
    ...
    def define_command(self, platform):
        ...
        cmd = "%s SamtoolsFlagstat -i %s -s %s > %s !LOG2!" % (qc_parser, input_file, sample_name, qc_report)
        return cmd
