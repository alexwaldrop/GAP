...
class ParseSamtoolsIdxstats(Module):

    def __init__(self, module_id):
        super(ParseSamtoolsIdxstats, self).__init__(module_id)

        self.input_keys     = ["idxstats", "qc_parser", "note", "nr_cpus", "mem"]
    ...
    def define_output(self, platform, split_name=None):
        ...
        summary_file = self.generate_unique_file_name(split_name=split_name, extension=".idxstats.qc_report.json")
        self.add_output(platform, "qc_report", summary_file)

    def define_command(self, platform):
        # Get options from kwargs
        input_file      = self.get_arguments("idxstats").get_value()
